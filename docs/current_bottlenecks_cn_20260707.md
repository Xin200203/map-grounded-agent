...
```mermaid
flowchart LR
    subgraph 修复前["修复前 (弱语义 n=48)"]
        A1["从未见过目标<br/>55-70% 的失败"] --> A2[见过但未承诺] --> A3[承诺被驱逐] --> A4[极少走到锚点]
    end
    subgraph 修复后["四波修复后 (C45 n=48)"]
        B1["从未见过目标<br/>~15%"] --> B2["见过未承诺<br/>18/40<br/>(多为低质候选)"] --> B3["承诺被驱逐<br/>~0 (C2消灭)"] --> B4["锚定却failed<br/>16/40<br/>← 失败质量堆在这里"]
    end
    修复前 -. "召回修复+C2+C4+C5" .-> 修复后
```
...
```mermaid
flowchart TD
    R["跨场景 SR 钉死在 10-17%"] --> C1["✅ 已解决的卡点"]
    R --> C2["❌ 剩余卡点"]
    C1 --> D1["图召回 (S1): 6→16 caption<br/>一行参数, 已验证"]
    C1 --> D2["承诺创建 (S3-5): C4 自动承诺<br/>在体 527 次触发"]
    C1 --> D3["承诺维持 (S6-7): C2 粘性<br/>病灶集 100% 翻转"]
    C1 --> D4["锚点精度: C5 分层<br/>幻影锚被门控"]
    C2 --> E1["🔴 S8 最后一公里 (16/40)<br/>锚定后无法完成 接近→看见→锁定→判停<br/>位于冻结 backbone 的 instance_discriminator<br/>= 改它就超出'控制器层'范畴"]
    C2 --> E2["🔴 感知质量残余 (18/40)<br/>合格候选(≥2次+高相关)不出现<br/>= 检测器/融合质量, 换检测器才能解"]
    C2 --> E3["🟡 统计层: 单集特异性极强<br/>n=12 信号五次在 n=48 死亡<br/>任何后续主张必须 n=48 配对+显著性"]
```
