...
**Bottleneck localization (GT-grounded, §4.6d).** Rather than infer the residual by elimination, we measure it. A per-episode classifier walks each trace and, with the GT goal position, records the deepest pipeline stage reached (committed-to-target / qualified-candidate / caption-recognized / approached-within-2m). On the reference (c45, 40 failures): the target is **committed in 36/40, a qualified candidate in 34/40, caption-recognized in 34/40** — target *finding* fails in only 4/40. The dominant failure (32/40) is **recognized-and-committed but the agent never navigates within 2 m of the true goal** (closest-approach axis validated: 7/8 successes ≤1.5 m). This overturns an elimination-inferred "perception-recall" reading: the residual mass is not in finding the target but in the *commit→arrival* gap. We split that gap with per-episode graph-node dumps and the GT goal position (axis flip + permutation calibrated on successes, consistent with the pose→GT calibration). The committed target is **not localized near the true goal**: a target-recognized node lies within 1.5 m of GT in only 3/75 failures (7/75 at 2.5 m), vs 10/21 successes (17/21 at 2.5 m) — an 8–9× discriminating gap. In 69/75 failures **no node of any kind** is mapped within 1.5 m of GT: the true goal region is never observed. Navigation failure (target mapped near GT but agent never reaches it) accounts for only 3–7/75. The measured bottleneck is therefore **target-instance disambiguation**: the system commits to a category-matching object that is not the described goal instance (36/40 failures commit to the category, 34/40 fire a qualified candidate), so the agent moves toward the wrong instance and the true goal is never mapped or approached. The four falsified fronts (controller §4.2–4.6, S8 funnel §4.6b, coordinate fusion §4.6c, target-text grounding §4.6d/R1) each operate orthogonal to this: none disambiguates *which* category instance is the goal. This is the leverage a text description is supposed to provide and the one the pipeline does not exploit — it commits to the first category match and never re-searches for a better-matching instance.
